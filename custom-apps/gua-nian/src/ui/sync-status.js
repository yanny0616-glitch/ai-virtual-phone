  function renderCloudSync() {
    ...
    let html = '<details class="card fold"' + (wasOpen ? ' open' : '') + '><summary><span class="t">云端同步</span><span class="sm">' + new Set(rows.concat(genRows).map(x => x.cx.character.id)).size + ' 个角色' + (rows.some(x => x.state.status !== "synced") || genRows.length ? ' · 有未完成项' : ' · 已同步') + '</span><span class="cv" aria-hidden="true">›</span></summary>' + rows.map(({ cx, state }) => {
