  ...
  const SET_SECTIONS = [
    ...
    { id: "cloud", name: "云端", groups: [
      ...
      { title: "主动消息的忙与睡", sub: "到点了TA顾不上呢", fields: [
        ...
        { type: "stepper", key: "busyMaxHoldMin", min: 30, max: 480, step: 30, label: "等待多久概率减半", unit: "分钟 · 不设固定截止，聊过或已解决就作罢" },
        { type: "seg", key: "sleepMode", options: [[0, "睡着就不发"], [1, "押到起床后"], [2, "概率醒来"]] },
        { type: "stepper", key: "sleepWakeProb", min: 5, max: 60, step: 5, label: "醒来的概率", unit: "%" },
      ], hint: "到点时TA正忙（上课、开会、开车这类，生成日程时标的），主动消息就押到忙完再加几分钟（按这个数上下浮动四成）；不设固定截止；每经过设定的分钟数，保留发送概率减半。忙完再核对最新聊天，已经提过或事实已变化就作罢。此规则需更新个人云函数。睡着了按选的办：不发、押到起床后、或按概率迷迷糊糊醒来说一两句。这里只管主动消息预约。<br>聊天里的延后回复已迁到小手机「设置 → 聊天插件 → 忙碌回复」：在那里设置偷空概率、等待间隔、睡眠和紧急消息规则。挂念只提供作息和忙闲，首次安装插件时会导入原设置，之后两边互不覆盖。启用个人离线推送后，聊天显示「等待已同步云端」才可关闭小手机。" },
    ] },
  ];
