...
const fixtureDir = path.join(root, "scripts/fixtures/shiguang-app");
...
await new Promise((resolve, reject) => webpack({
    ...
    entry: path.join(fixtureDir, "browser.ts"),
    output: { path: out, filename: "test.js" },
    resolve: { extensions: [".tsx", ".ts", ".js"], alias: {
        [path.join(root, "lib/shiguang-summarizer")]: path.join(root,"scripts/fixtures/shiguang/summary.ts"),
        [path.join(root, "lib/short-term-assembler")]: path.join(root,"scripts/fixtures/shiguang/timeline.ts"),
        [path.join(root, "lib/chat-storage")]: path.join(root,"scripts/fixtures/shiguang/chat.ts"),
        [path.join(root, "lib/character-storage")]: path.join(fixtureDir,"characters.ts"),
        [path.join(root, "lib/kv-db")]: path.join(root,"scripts/fixtures/shiguang/kv.ts"),
        "@": root,
    } },
...
await fs.writeFile(path.join(out,"index.html"),`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0}</style></head><body><script>window.appHtml=${JSON.stringify(appHtml).replaceAll('<','\\u003c')};</script><script src="test.js"></script></body></html>`);
