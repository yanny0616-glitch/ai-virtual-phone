...
const picturedState = structuredClone(state);
picturedState.noteData.data.commentData.comments.push({user:{nickname:'图片评论'},content:'',pictures:[{originUrl:'http://sns-img-qc.xhscdn.com/comment/a',url:'http://sns-na-i11.xhscdn.com/comment/a?w=360'}],subComments:[{user:{nickname:'图片回复'},content:'',pictures:[{url:'http://sns-na-i11.xhscdn.com/comment/b'}]}]});
const pictured = reader.parseXhsNote(page(picturedState), parsed.url);
assert.equal(pictured.commentImageCount,2);assert.equal(pictured.images.length,5);
assert.equal(pictured.images[3].commentIndex,2);assert.equal(pictured.images[4].commentIndex,3);
assert.equal(pictured.images[3].url,'https://sns-img-qc.xhscdn.com/comment/a');
assert.match(xhs.formatXhsNoteSnapshot({status:'ready',note:pictured}),/图片评论.*附件第 4 张/);
assert.match(xhs.formatXhsNoteSnapshot({status:'ready',note:pictured}),/图片回复.*附件第 5 张/);
...
const snapshot = { sourceUrl: parsed.url, status: 'ready', note: { ...pictured, images: pictured.images.map((image, i) => ({ ...image, ref: `data:image/png;base64,IMAGE${i + 1}` })) } };
...
for (const group of [false, true]) for (const chronological of [false, true]) for (const vision of [false, true]) {
    ...
    assert.equal(images.length, vision ? 5 : 0, JSON.stringify({ group, chronological, vision, result }));
...
assert.equal(message.mediaData.xhsNote.note.images.length, 5);
