...
function load(file, globals, expose) {
    ...
    const ctx = vm.createContext({ console: quiet, Date, URL, Request, Response, Headers, Buffer, AbortController, AbortSignal, TextEncoder, TextDecoder, Uint8Array, setTimeout, clearTimeout, ...globals });
...
function chatHarness() {
    const disk = { messages: new Map(), sessions: new Map() };
    const state = { failAt: '', ackFail: false, entries: [], acks: [], transforms: 0, hooks: 0, published: 0 };
    let tx = null;
    class Dexie {
        version() { return { stores: () => { for (const name of ['messages','sessions']) this[name] = {
            put: async row => { (tx ?? disk)[name].set(row.id, clone(row)); },
            bulkPut: async rows => { for (const row of rows) (tx ?? disk)[name].set(row.id, clone(row)); if (tx && state.failAt === name) throw new Error('QuotaExceededError'); },
        }; } }; }
        async transaction(_mode, _messages, _sessions, fn) {
            tx = { messages: new Map(disk.messages), sessions: new Map(disk.sessions) };
            try { await fn(); disk.messages = tx.messages; disk.sessions = tx.sessions; } finally { tx = null; }
        }
    }
    const db = load('lib/chat-db.ts', { Dexie }, 'dbPutMessage,dbPutSessions,dbPutMessageBatch');
...
for (const failAt of ['messages','sessions']) for (const bridge of [false,true]) await test(`Outbox ${bridge?'bridge':'normal'} ${failAt} failure rolls back whole batch; retry publishes once`, async()=>{
...
await test('New page after failed transaction can import the complete cloud batch', async()=>{
    ...
    const fresh=chatHarness(); fresh.chat.seed(fresh.session,[...old.disk.messages.values()].map(clone)); fresh.state.entries=clone(old.state.entries);
...
await test('Existing memory-only batch is not acknowledged while persistence fails', async()=>{
    ...
    h.state.failAt=''; await h.outbox.consumeServerOutbox({force:true}); assert.deepEqual(h.state.acks,['e']); assert.equal(h.disk.messages.size,1);
});
