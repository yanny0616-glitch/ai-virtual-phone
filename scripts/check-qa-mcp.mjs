...
const context=vm.createContext({console,JSON,Error,
 ...
 getQaPageChars:()=>2000,
 callConfiguredMcpTool:async(server,name,args)=>{calls++;if(failure)throw failure;assert.equal(server.accessToken,'private-token');assert.equal(name,'read');assert.equal(args.page,1);return response;},
});
...
failure=new Error('MCP tools/call（401）：Missing bearer token private-token');
await assert.rejects(run(args),err=>err.message.includes('401')&&err.message.includes('OAuth')&&!err.message.includes('private-token'));
failure=new Error('Invalid params: name too long');
await assert.rejects(run(args),/Invalid params: name too long/);
failure=null;
