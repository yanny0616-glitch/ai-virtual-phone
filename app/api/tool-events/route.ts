...
export async function POST(req:NextRequest){
 ...
 try{
  ...
  const supplied=Buffer.from(req.headers.get("x-float-events-key")||req.headers.get("x-float-garden-key")||"");
  ...
  if(!["status","save","start","stop","clear","events","ack","credentials"].includes(body.action))throw Error("未知操作");
