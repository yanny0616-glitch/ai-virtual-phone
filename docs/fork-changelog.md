...
- 验证：`npm run gua-nian:build`、`npm run check:apps-dist`、`node scripts/check-gua-nian-diagnostics.mjs`（补了同步/记录渲染钩子桩），430px Chromium 截图核对无云、有云、全部展开三种状态（演示数据）。攒批发版，manifest 版本未动。
