...
## 2026-09-14：悬浮球贴边坐标恢复

- 两个球都按容器布局尺寸校正停靠坐标并监听尺寸变化；忽略无法容纳球的瞬时尺寸，纵向避开状态栏。单球恢复共享锚点，定位标记跟随实际 left/top。
- 坐标专项检查通过（顶部遮挡、宽屏旧坐标、瞬时高度、横竖屏切换）；目标文件 lint 与修改前一致，仍有 14 个原有错误和 5 个警告。未跑本地全量构建，未做手机完整验证。

## 2026-09-14：外观预设与开屏动画

- 外观 APP 新增「外观预设」：整套外观存多份、一键切换，素材只记引用；删素材与恢复默认不动预设仍引用的素材。
- 外观 APP 新增「开屏动画」：漂浮 / 墨色 / 极光 / 脉冲 / 不要开屏，本设备生效，随备份带走。`tsc` 通过；外观页原有 5 个 lint 错误未扩大处理。
- 自定义开屏：贴自己的 HTML/CSS/JS 或导入 .html，沙盒 iframe 运行，存多套可切换。

## 2026-09-14：工坊可调 REST / 组合 / 自定义 APP 工具

- 工坊配置加第二张授权勾选表，默认空；REST 换地址、组合工具被改、APP 升版后授权自动失效。
- 新固定工具「调用工具箱工具」，按 id 定点执行，REST 凭据脱敏。回归 `check-qa-tool-access.mjs` 接入 `check:qa`。

## 2026-09-14：工坊提案 diff、语法高亮、重新生成

- 提案卡文件行可展开看逐行 diff（新增 `lib/qa-diff.ts` + `check-qa-diff.mjs`），确认模式下提案时读原文；全自动模式不读。
- 代码块接 highlight.js（core + 11 种语言，新依赖），颜色跟浅色 token。
- 最后一条回复可「重新生成」，复用编辑重发的副作用门禁。`tsc` 通过，`check:qa` 四个脚本通过。未提交。

## 2026-09-14：工坊五项修补

- 上下文计量补算图片，带图会话能正常触发压缩。
- 输入框 Enter 发送（桌面），Shift+Enter 换行，触屏保持换行。
- 工坊配置新增「采样温度」，默认 0.8、留空不传；不走聊天预设。之前固定 0.8 无处可调。FAQ 改口。
- 创作指南退出 system prompt 注入，只走「创作指南」工具，新增 `plugin` 类型。
- 新增 `check:qa`（引擎过滤器/上下文回放 + toolbox + MCP 三个脚本），接入 CI。`tsc` 通过；`phone-qa-app.tsx` 原有 2 个 set-state-in-effect lint 错误未扩大处理。未提交。

## 2026-09-12：挂念 0.9.38 设置遮罩修复

- 设置/详情打开样式只匹配原 #sheet-mask，避免激活位于设置上方的提示词专用遮罩。字体保持原13px。
- Chromium验证设置打开、提示词打开/关闭、草稿取消与完成；新增 scripts/check-gua-nian-prompt-browser.mjs。


## 2026-09-12：挂念 0.9.37 提示词编辑弹窗（随 0.9.37 发布）

- 配置入口改为编辑按钮，独立底部弹窗沿用原APP颜色、字体和控件；保留父设置草稿，关闭取消、完成回填、设置页保存生效。
- 增加焦点管理及键盘关闭；用真实生成HTML在390px Chromium中检查展示和草稿交互，无云函数改动。


## 2026-09-12：挂念 0.9.36 可编辑日程生成提示词（随 0.9.37 发布）

- 配置基本页增加日程生成提示词，默认描述角色独立生活及用户自主行动边界；支持编辑、保存和恢复默认草稿，空白保存回退默认。
- generateDay 与云端 genKit 共用 buildDayInstruction，保存变化使原料上传节流失效，下一次上传刷新；既有日程不自动重建，无云函数/schema变化。
- 设置专项及本地/云端真实请求模拟通过；未调用真实模型、未做手机完整实测。



## 2026-09-12：陪眠 0.3.3 三项数据修复（随 0.3.3 发布）

- 同晚再次睡眠复用夜记，睡眠区间累计排除已记录清醒段；旧重复归并展示并标记已合并原始行，保留原件。周/月获得统一单晚数据。
- 清空等待在途写入、使旧任务失效，按批删完记录并重建 settings；默认对象深拷贝，周评、污染的声景与500行后残留不再恢复。上传/导入、周评和早安迟到回写增加取消校验。
- 重复点击合并入层意图，下载完成后重新校验 key/六层上限与当前组合；保存的重复层读入时归一。
- 专项覆盖真实会话、存储、并发入层和清空边界；仅处理用户指定三项，其他审阅发现仍待处理。新增APP版本及安装包，无新增宿主/云函数接口。


## 2026-09-12：陪眠 0.3.2 前两项审阅修复（随 0.3.3 发布）

- 宿主 voice.play 在媒体读取前登记通道 revision，stop/新播放/卸载使旧请求失效；迟到媒体与旧播放失败不启动旧音频、不清理新音频，正常结束仍释放临时 URL。
- 试一句冻结角色、情绪和语音开关；离页/换角色/正式会话开始时取消，生成和 TTS 返回后复核；正式会话等待已有试读停止，停止失败可重试。
- 针对性测试覆盖两项原始复现和取消边界。只处理全量审阅 F01/F02，其他发现仍待处理。更新宿主及陪眠安装包，无云函数变更。


## 2026-09-12：陪眠 0.3.1 会话与定时修复（随 0.3.3 发布）

- 5/10 分钟渐弱改为最多 20 秒一段，不再生成超过 25MB 的整段媒体；上传/播放失败停止原循环，取消中的旧渐弱不停止新播放。
- 入睡会话对象贯穿异步链路，停止期间串行等待清理；迟到生成/TTS/STT 不进入新会话。绝对截止时间同时停止人声、环境音和识别，保存最后一句，不填造醒来时间。
- `voice.stopSTT` 用 app 与 requestId 隔离，松手调用底层 STT stop 等最终文字，取消则 abort 并丢弃文字；不改 voice.stopRecord 的独立录音行为。
- 新增模拟宿主/时钟专项，验证旧会话、15 分钟截止、迟到 TTS、STT 松手取消、媒体大小和渐弱失败兜底；领域测试和产物检查继续保留。未做真实手机锁屏整夜验证；系统完全挂起 JS 时不保证准点停止。宿主和 APP 须一起更新，无个人云函数变更。


## 2026-09-12：同步上游 fc65539

- 合并上游 9 个提交。角色编辑初始化同时保留本地人设折叠和上游拍立得参数；角色卡导出同时保留卡内世界书与拍立得字段。
- SW 缓存升至 v38，使本次上游样式更新生效。角色卡世界书及拍立得导入导出联合验证、发布产物一致性检查通过。
- 角色编辑文件 lint 的 11 项错误均可追溯到合并前或上游既有代码，未在此次同步中扩大修复范围。构建由 GitHub Actions 执行。


## 2026-09-12：云端普通回复恢复预设思维链折叠（随宿主发布，个人云需更新）
...
## 2026-09-12：挂念云端发送前解析思考标签（随宿主发布，个人云需更新）
