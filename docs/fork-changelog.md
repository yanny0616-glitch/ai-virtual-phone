...
## 2026-09-17：长期总结改为正常叙述，记忆卡片保留换行

- 上一版提示词要求每段「M月D日 HH:MM · 事件：经过和结果」，模型每段都先起小标题（「深夜求陪：」「留灯：」）。改为正常叙述、一件事一段、段间换行，日期时间自然写进句首，不加小标题。上一版默认存进配置的自动换新（`EVENT_TITLE_SUMMARIZATION_PROMPT`）。
- 记忆页长期 / 核心记忆卡片正文加 `white-space: pre-wrap`：之前换行被吞，几段连成一整段。
- 验证：`check:memory-prompts`（新增上一版迁移）、根 `tsc --noEmit`。
...
- 上传单次 `timeout 300`，失败最多重试 3 次（间隔 20 / 40 秒）；每次上传前后删掉所有 `float-build-*` 草稿（中途取消 / 挂住留下的残骸，同一并发组里不会有正在传的）。
- 修 bug：保留最近 3 个的清理把草稿也算进去，残留草稿占位时会删掉正式版。9/17 就因此只剩 c02b956 一个正式版（本机 `/opt/float/releases` 的回滚目录不受影响）。现在排除 `isDraft`。
- 验证：`bash -n`、js-yaml 解析；两个 jq 过滤器对线上 release 列表只读试跑。

## 2026-09-17：长期总结按事件合并、每批字数可调

- 问题：阶段一的总结提示词（普通小事也记 + 原话带引号标说话人 + 按分钟分段 + 不为字数丢原话、无长度约束）让模型逐条转述，写出来比原记录还长，还夹着大量「记录中没有……」「未显示……」。
- 新默认提示词：按事件合并成段（只标开始时间），原话只留称呼、承诺、第一次说出口的话和有分量的一句，闲聊小事几件合一句，不写「记录中没有……」，全文不超过 `{{words}}` 字，写不下先压缩闲聊、保住时间约定和关键原话。上一版默认存进配置的自动换新（`PHASE1_SUMMARIZATION_PROMPT`）。
- 记忆设置「自动总结间隔与字数」新增「每批总结字数」滑块（100～2000，默认 500，`summaryWordLimit`），填进提示词 `{{words}}`；自己改过的提示词没写 `{{words}}` 时末尾追加一句字数上限。
- 验证：`check:memory-prompts`（新增 `{{words}}`、按事件、阶段一迁移）、根 `tsc --noEmit`。未做手机端实测。

## 2026-09-17：记忆阶段二 2.1 短期原话窗口（二选一）

- 记忆设置「少带重复内容」新增「聊天原话带多少」：**原来的做法**（默认，按短期预算从新往旧塞满）/ **总结之后 + 近几天**。
- 新模式：`buildChatPromptMessages` 给 `prepareShortTermContext` 传 `notBefore` = 长期总结水位线往前 N 天（`shortTermWindowStart`，默认 3 天，0～14 可调），早于它的聊天和事件不进短期记忆，之后仍按预算兜底。起点锚在水位线而不是当前时间，两次总结之间不动，提示词前缀稳定；总结落后时窗口自然变大、由预算截住，等于退回原做法。
- 不生效的情况：还没总结过（无水位线）、线下模式（历史是当前场景本身）、群聊装配。
- 验证：`check:memory-layering`（新增窗口起点）、`check:memory-prompts`、`check:memory-recall`、`check-custom-app-prompt-context`、根 `tsc --noEmit`。未做手机端实测。

## 2026-09-17：长期记忆总结分批、显示进度

- 问题：手动总结（和自动总结）把水位线之后的全部记录一次喂给模型、写成一条。删光长期记忆后水位线清空，「接着上次总结」会把整个历史写成横跨几周的一大条，按话题挑不细，核心去重和 2.1 的时间边界也不准，还容易被截断整次作废。
- 现在按「自动总结间隔」（默认 80 条）分批，每批一条长期记忆、各自推进水位线；同一时刻的记录不拆到两批，末尾不足 4 条并进上一批（`splitSummaryBatches`，`lib/memory-layering.ts`）。某批失败就停，已完成的批次保留，提示「前 N 批已保存，第 N+1 批失败」。
- 进度改为全局通知（`components/memory-summary-toast.tsx`，挂在桌面壳根部、状态栏下方，和微信同步 toast 同位置）：自动和手动总结都显示「正在整理 X 的记忆：第 2/5 批」，常驻到结束，结果条（完成 / 失败 / 已停止）到点消失。
- 停止：通知条和记忆页的按钮（总结中变成「停止」）都能停，当前这批写完再停，不会半批入库或重复总结（`requestStopSummarization`）。
- 记忆页说明行多批时显示「正在总结第 2 批，共 5 批」，每批入库后列表即时刷新。
- 验证：`check:memory-layering`（新增分批场景）、`check:memory-prompts`、`check:memory-recall`、根 `tsc --noEmit`。未做手机端实测。
...
- 前几天的记录加上当天睡眠和起床精力；新增规则：前几天没睡好不自动延续，昨晚没有新原因时睡眠好转、起床精力逐天回到 70–90，连着偏低的要明显回升；偶尔没睡好照样可以写，但要有昨晚自己的原因。
- 后端 `companion-server/src/day.ts` 与挂念 App `planning/generation.js` 同步修改，安装包版本号未改。明天起新生成的一天生效。
...
- 换版本整轮替换：正文、摘要、思考一起换，线下摘要同步事件照常发出（挂念读到的摘要跟着变）。之后又聊一轮或删改轮次，其余版本作废。
- 行为变化：线下重试失败或中途停止时，自动放回原来那一截（以前只把输入放回输入框、回复丢失）。
- 新增 `lib/chat-offline-reroll.ts`，验证：`check:offline-reroll`、根 `tsc --noEmit`。未做手机端实测。
...
## 2026-09-17：核心记忆保留普通细节、长期记忆不再删除

- 核心记忆默认提示词改回「再精炼、不丢信息」：核心是长期记忆太多、旧条目带不进提示词时的兜底，普通细节（常聊话题、习惯、近况、反复出现的小事、代表性原话、一时情绪的起因结果）压短保留，只省重复寒暄和无依据推测；删掉上一版「这些已经留在长期记忆里，不会丢」的错误说法；按时间写、带日期，不设字数上限。上一版默认存进配置的同样自动换新。
- 长期记忆超过 `maxLongTermEntries`（500）不再删除最旧条目，只影响注入量。
- 验证：`check:memory-prompts`（新增上一版迁移、核心保留普通细节）、根 `tsc --noEmit`。

## 2026-09-17：记忆阶段一修复（提示词、token 校准、占用提示）

- 长期总结默认提示词重写（`lib/memory-types.ts`）：绝对时间分段、关键原话与称呼标明说话人、约定写清时间条件与是否兑现、普通闲聊小事也要记、改变的事写最新说法、禁用空泛动词、长短随内容；新增占位 `{{count}}`（本批条数）。
- 核心记忆默认提示词重写：「要保留 / 要略去」分开写（旧版两类混在一个列表里）；没有值得写进核心的内容时输出「无」，不入库只推进进度。
- 迁移：`loadMemoryConfig` 读到和旧默认全文一致的提示词自动换成新默认；改过的不动，设置页提示「恢复默认」会覆盖。
- token 按模型校准（`lib/token-calibration.ts`）：每次成功调用用「原公式估算 ÷ 实际 prompt_tokens」按模型更新比例（含图片的请求不算）。目前只用于显示，预算截断仍用原公式。
- 记忆页角色详情顶部新增「上次请求记忆占用」：短期原话 / 近期事件 / 长期 / 核心 / 拾光各占多少 token，点开看条数和字数；数据在聊天请求装配时记录（`lib/memory-prompt-usage.ts`），预设里关掉的层记 0。
- 行为变化：之后自动 / 手动总结出的长期、核心记忆格式会变（分段日志、可能更长）；发给聊天模型的其他内容不变。
- 验证：新增 `check:memory-prompts`（提示词占位、迁移、校准）、`check:memory-recall`、根 `tsc --noEmit`。未做手机端实测。
- 更正方案文档：长期提示词末尾并没有写错成「核心记忆总结：」，删掉该条。
...
- 小手机 `lib/offline-jobs-client.ts`：`push-bailout-client` 的挂 / 撤 / 心跳都经它分流。同一 triggerKey 只挂一边：挂之前先撤另一边，撤不掉不挂，另一边正在生成就让它发完；撤销和心跳两边都发。模板、快捷动作续跑、挂念旧本机预约仍走个人云。
- 后端到点再查个人云同名任务：正在跑等 1 分钟，比这条新或挂上后已处理就不发，更早的待发就撤掉。
- 离线执行页文案改为全部任务跟随开关。
- 验证：后端 104 项测试（新增 10 项离线任务）+ tsc；根 `tsc --noEmit`。未做手机端实测。
...
- 宿主：`push.freeze` 支持 `chatSnapshot`，冻一份聊天 APP 原样提示词 + 完整聊天记录的聊天模板，意图和「多久前决定的」留占位由后端到点填，冻结时的系统时间 / 角色本地时间也由后端换成发送时刻；已登记模板在 TA 回复后 30 秒（原 3 分钟）和切到后台时重冻。
- 挂念 0.9.42：设置「云端」新增「交给 VPS 后端」，开着时只寄判断 / 生成一天 / 聊天三份模板，不生成、不复核、不排预约（避免与后端重复发送）。
- 顺带发现：旧哨兵快照把「挂念后台复核模板，仅供后台调用，不生成聊天消息」烤进了定时唤醒指令的「你当时想着」，云端以前的主动消息都带着这句和冻结时的时间；后端对旧快照也做了替换。
- 验证：`companion-server` 20 项测试；`gua-nian:check`、`check:apps-dist`、根 `tsc --noEmit`；后端已真发一条到手机。

## 2026-09-16：挂念 0.9.41 事项去重（随 0.9.42 发布）

- 本机与云端复核统一事项编号，合并相似念头与约定的预约归属；明确约定优先，已经发送但缺少新进展证据时阻止再次跟进。
- 发送端核对当前事项与已发送输出，成文期间被去重的任务不交付；复用原模型调用进行语义核对，未新增设置或修改正文预览。
- 新增纯规则及真实 APP/云端复核/发送 worker 专项；同源副本、轻量产物及 lint 验证。需更新挂念和三个个人云函数，无新 SQL；未做手机端实测。
...
- 新增小卷工具「出主题」：不直接改页面，出一份方案并弹卡片；`css` 与 `patches` 二选一，补丁的 `find` 必须在当前 CSS 里恰好出现一次，不唯一就整份拒绝。
- 卡片（`components/mascot/mascot-css-plan-card.tsx`）：预览 / 结束预览 / 应用 / 撤销 / 只存进主题库 / 看代码，行级增删统计；主题库可套用（同样走方案管线，可撤销）和删除。挂在 `MascotPreviewHost` 上，桌宠收起时也弹得出来。
- `读取CSS` 返回里加「主题地图」：顶层块按顺序编号 + 选择器 + 行数，外加已声明的 CSS 变量清单，小卷照着写补丁定位片段。
- `覆写CSS` 保留原样（整段替换、立刻生效），提示词把默认路径改成「出主题」。
- 验证：`tsc --noEmit` 通过；`npm run check:qa` 通过；改动文件 eslint 错误数与改动前一致。
...
- `MomentPost` 新增 `photoUrls`，第一张仍写 `photoUrl`，只读单图的调用方（查手机、短期记忆、生图重试）不用改。
- 多图帖子把整组配图都带给模型（开了图像识别时，上限 9 张），逐张编号「朋友圈配图 N：」，正文写「共 N 张」。
- 存储占用统计和按天清理按整组算，`photoUrl` 与 `photoUrls` 一起清。
- 聊天里发照片也能多选，按顺序连发，描述只跟第一张。
- 验证：`tsc --noEmit` 通过；改动文件 eslint 错误数与改动前一致。

## 2026-09-15：语境属性、时辰主题、通知横幅版式

- 手机屏幕和聊天室外层自动挂 `data-time-of-day`（早上 / 白天 / 傍晚 / 深夜，每分钟校一次）、`data-hour`、`data-color-scheme`，会话 CSS 写 `:root[data-time-of-day="night"] …` 就能换一套配色。
- 聊天列表项、朋友圈帖子、两种通知横幅都挂上语境属性：未读多少、置顶 / 免打扰 / 群聊、最后一条的类型和时间、预览长短、有没有配图、点没点赞、通知是消息还是来电；列表项另给 `--item-index` `--item-unread` `--item-avatar-url`。
- 通知横幅加一整套版式变量（`--notif-layout` 等 20 个），能换成右图左字或上图下字的拍立得样式，位置、宽度、圆角、头像尺寸、「查看」按钮位置都能调。
- 修 `lib/css-scoper.ts`：`:root` 紧跟属性或伪类的选择器之前不生效。
- 契约清单写进 `lib/theme-types.ts` 顶部注释；属性名发布后不改名。
- 验证：`tsc --noEmit` 通过；改动文件 eslint 无新增错误。

## 2026-09-15：回复方式、聊天截图、通话记录、通话细节

- 聊天信息 ›「聊天」加回复长度、整段一个气泡、线上 `*动作*`、自动切换线上线下（`[切到线下]` / `[切到线上]`，顶上提示可撤销）。
- 多选消息「截图」：克隆真实气泡出长图，自定义 CSS 保留；选项、80 条上限、外站图片提示；生成后点分享存相册。
- 通话记录页：列表 + 详情，TTS 回放、改字、合成 WAV 分享、修复中断通话、删除；挂断后自动一句小结，卡片显示，长期记忆总结用小结替代原文。
- 聊天信息新增「通话」一栏：旁白、TA 挂断、小结、默认开摄像头、场景跟随在线状态、立绘和场景库、通话美化 CSS（直播风预设、挂断键保护）；通话界面加稳定 class 钩子。
- 预设新增 `reply_style` `call_extras` `call_summary_prompt`，补丁号 3；新依赖 `modern-screenshot@4.7.0`。
- 验证：`tsc --noEmit` 通过；改动行 eslint 只有 2 条 no-img-element 警告（图来自 IndexedDB / blob，用不了 next/image）。

## 2026-09-15：「+」面板自订、聊天变量、双向拉黑

- 「+」面板右下「编辑」：勾选要显示的、按住拖动排顺序，没勾的收进「显示其余 N 个功能」，全局一份；新装 APP 按钮默认勾上、带红点；插件按钮固定在后面。
- 「聊天变量」面板（「+」面板 / 聊天信息进）：和插件变量池同一份，状态栏按模式可改 / 只看 / 不显示，官方插件和挂念的变量有中文名和摘要；用户变量可写规则，开 AI 后按 `[变量 名=值｜理由]` 每轮改，记最近 20 条变化；`{{setvar}}` 落进变量池不再丢。
- 双向拉黑：角色写 `[拉黑:…]` / `[删除好友:…]` 后用户消息被拒收（红 !，不进上下文），冷静期后角色重新考虑，被删可发朋友验证；用户拉黑角色时角色消息截进收件箱可偷看，解除后角色知道。聊天信息新增「拉黑与删好友」一栏。
- 预设新增 4 条（`chat_variables` `chat_block_actions` `block_reconsider_prompt` `friend_verify_prompt`），补丁号升到 2，不动已有条目。
- 验证：`tsc --noEmit` 通过；改动行 eslint 0 条。

## 2026-09-15：约见面插件、聊天信息按类折叠

- 新官方插件 `meetup`（约见面）：「+」面板里约TA（现在 / 今晚 / 挑个时间，地点和想做什么可空），发一张卡片并让TA回一轮。TA 按自己那时的日程（作息、当天忙时段、此刻状态）在回复末尾写 `[约见答应]` / `[约见改时间:…]` / `[约见推掉]`，插件截掉标记改卡片；改时间时卡片划掉原时间，你点「就这个时间」才算约定。TA 也会主动约（`[约见:时间|地点|想做什么]`），分关 / 偶尔 / 常常，另有「最多几天约一次」「推掉后几天不再约」两道冷却；TA 的卡片等这一轮气泡放完再出现。
- 约定后按「到点叫你」（推送 / 只变卡片 / 不叫，可提前 N 分钟）走宿主定时唤醒，App 关着走离线推送；卡片到点变「去赴约」，点了切进线下并注入这次见面的开场。线下顶部一条「这次见面 · 散场」，散场后用辅助模型写一句回执，卡片盖「已赴约」章。
- 卡片三种样式（票根 / 简约 / 信笺），类名都以 `mt-` 开头，可在聊天信息 → 外观 → 自定义 CSS 里改；卡片用 `:has()` 去掉宿主气泡的底色和内边距。全局设置在插件管理页，每个角色可在聊天信息 → 约见面里单独设。
- 宿主新增 `ctx.chat`（`requestReply`、`offline.get/set/turns`、`scheduleWake/cancelWake`）和 `chatInfo.section` 坑位；`chat.header` 坑位带 `offlineMode`。插件唤醒不占普通唤醒「一会话一条」的名额。
- 聊天信息页除备注 / 查找 / TA 的电脑 / 群成员外按类折叠：聊天、生成、插件各一栏、状态栏、外观、清理与删除，一次只展开一类，标题下一行是当前状态摘要；插件栏什么都不画（如群聊）就不显示。
- 验证：`npm run check:meetup-plugin`（17 组时间写法、6 组标记截取）通过；`tsc --noEmit` 通过；改动文件 eslint 0 error（9 条既有 warning）。卡片三种样式装进生产构建（旧宿主）截图确认；折叠页和 `ctx.chat` 相关流程本地未构建，要等新宿主上线后在手机上核对。

## 2026-09-15：经期按记录自动算、前一晚提醒

- 周期设置新增「按记录自动算」（默认开）：取最近 6 次开始日之间的间隔，丢掉 18～60 天以外的和比最短一次长 1.6 倍以上的（多半中间漏记了一次），再丢掉离中位数太远的，剩下的按新近加权求平均周期；波动取偏差中位数（1～4 天，不到 3 段按 2 天）；经期天数取记录中位数。记录不够两次或关掉开关，就用手填的两格（此时不再变灰）。设置页显示平均周期 / 波动 / 经期、间隔柱状图（丢掉的画斜线并划掉数字）、平均虚线和下次窗口。
- 预测从一个日子改成一段窗口：最可能那天 ± 波动。月历上窗口连成浅玫瑰色带、跨周断开，最可能那天实心圆；详情页在窗口开始前 10 天内显示「3–7 天后 · 最可能 20 日 周日」，窗口里显示「随时可能来」，今天已过窗口还没来显示「晚了 N 天」。晚了不往后跳周期，直到下一个窗口开始才滚动。
- 关心时机改按窗口算：提前 N 天、到最可能那天、晚了 3 天（key `<peak>:late`）各一次。新增「前一晚提醒」：不提醒 / 21:00 / 21:30 / 22:00 / 自定，窗口开始前一晚由选中角色按人设发一条（key `<peak>:eve`），和提前 1 天的关心撞在同一天时只发这条。App 开着时轮询在提醒时间后 3 小时内补发；App 关着走已有 timed_task 兜底（7 天内才预约，免打扰时段跳过），推送回执沿用 `periodCare` 元数据，云函数不用改。
- 验证：`npm run check:menstrual-predict`（7 组：漏记剔除、记录不足退回手填、关开关、波动宽窄、晚了不跳、往后投影、关心时机与前一晚去重）通过；`tsc --noEmit` 通过；改动文件 lint 与改动前一致（日历页 2 个、月历页 1 个、follow-up 4 个既有 error，经期存储 1 个既有 warning）。样稿在生产构建上注入截图确认；本地未构建，真实页面待上线后核对。

## 2026-09-15：长期记忆可选「按话题挑」

- 参考糯叽机的向量召回闸门。记忆设置新增「注入方式」：「全部放入」是原来的做法，默认；「按话题挑」每轮只挑和眼下话题相关的长期记忆，另有「每轮最多几条」（1～20，默认 8），同时不超过「长期记忆」预算。
- 打分：关键词一路总在（CJK 双字切分 + BM25；按至少 20 条算 idf；出现在六成以上记忆里的词和「用户」「角色」不参与），配了向量模型并开着「向量召回」再并一路向量，查询嵌入 3 秒超时就只走关键词。再加重要度和新近度，过闸（关键词分 ≥ 1 或余弦 ≥ 0.4）、低于本轮最高分四成的丢掉、近重复只留一条。两条以内不设闸。
- 选中的按发生先后排，每条前标日期和「约多久前」，时间取总结时记下的 `timeSpan`，没有就用创建时间。标签只挂在返回副本的 `metadata.recallLabel` 上，所有调用 `formatLongTermMemories` 的地方自动带上。
- 检索当轮进行，用的就是刚发的那句。线下模式的当前场景原本被排除在时间线外（本轮剧情走 history），「按话题挑」时把本次请求最近 8 条消息并进检索词（`mergeRecallContext`，聊天引擎调用处经 `buildLongTermRecallContext`）；「全部放入」不变。
- 已知短板：只靠关键词时换了说法就对不上（「小咪」和「那只猫」），配向量模型能补上。
- 验证：`npm run check:memory-recall`（10 组：命中排序、闲聊不召回、条数上限、去重、向量同义与维度不符、新近度、极少记忆、角色名泛滥、日期标签、线下本轮消息并入检索词）通过；`tsc --noEmit` 通过；改动文件 lint 与改动前一致（记忆页 4 个既有 error）。设置页样稿按宿主现有组件注入截图确认；本地未构建。

## 2026-09-14：重新生成附说明与版本切换、申请持久存储

- 参考糯叽机的「附说明重 Roll」和「版本选择器」。长按角色回复点「重试以下」，先弹「重新生成」框：六个快捷问题标签（角色 OOC、忘了前文、搞错事情、太短、太长、重复前文）、自由说明、「我的常用」可存可删，可选「附上上一版给 TA 对照」。什么都不填按钮显示「直接重来」，行为与原来一致。
- 说明拼成一条系统指令，只跟这一次请求走，不落库、不在聊天里显示；单聊、群聊都认。附上一版时截取前 1500 字。
- 最新一轮回复重来过就保留版本（每个会话一组，最多 8 版，存 kv，媒体清理扫得到其中的图片引用）。长按这轮回复，菜单多一项「换一版 k/n」，打开「选一版」列表点选即换；发下一条消息、发系统指令或锚点消息被删后自动作废。重试更早的回复仍删掉后面全部消息，不留版本。
- 换版本 = 删掉当前这轮再放回目标版本（`restoreChatMessages`）。放回时换新 id、清掉 `cloudSync`：微信云同步已把旧 id 记成删除，带「来自云端」标记会被上传筛掉。聊天镜像、推送桥、微信云同步都按删除加新消息跟上。
- 启动时若浏览器还没给持久存储，调用一次 `navigator.storage.persist()`，降低空间紧张时 IndexedDB 被整库清掉的风险。
- 验证：`tsc --noEmit` 通过；聊天页 lint 改动前后同为 40 个既有 error，新文件零报错。样稿在生产构建上注入截图确认过版式；本地未构建，真实交互待上线后在生产构建上核对。
...
## 2026-09-13：小剧场接宿主流式（随 0.1.1 发布）

- 设置 → 生成新增「边写边显示」（默认开）：`ai.generate` / `ai.chat` 带第二个参数 `{ onChunk }`，关掉或老宿主则不带、行为不变。
- 纯文字逐字写进 iframe（`postMessage({type:"text"})` 整段重设，不重载，末尾光标闪动），尾随节流 160ms；增量里未闭合的 `<think>` 也先剥掉；出现 HTML 段只报字数，结束后正常挂载。
- 分两步出的第二步：每个增量 `parseFill` 出槽位就地填入，状态显示「正在填第 k / N 处」；结束以整段响应为准，`refreshResultMeta` 只刷标题和字数。
- 回归测试新增流式用例（预览剥 think、分两步增量槽位、关闭时不带参数、取消后丢增量）；两个无头 mock 的 SDK 会分段吐字，截图新增流式中一张。

## 2026-09-13：小剧场悬浮操作条、取消计时、HTML 分两步出（随 0.1.1 发布）

- 舞台三个按钮移出滚动区做成底部悬浮条，结果卡上移到本期卡下面，四个主题各自定样；「随机一篇 / 让 AI 自选」缩成「随机 / AI 自选」。
- 生成中显示已等秒数和取消；取消后旧调用的返回一律丢弃（token 递增），宿主没有中断接口。
- 组合输出新增「分两步出」（仅 HTML；设置 → 生成里可设新建默认值）：第一步只要带 `data-slot` / `data-hint` 占位的骨架，拿到即渲染（占位显示为骨架闪条）；第二步按 `[[编号]]` 填字，`postMessage` 就地填进 iframe，不重载。两步共用同一次抽中的随机；模型没写占位就按一次性成品处理；第二步失败保留骨架可重试。宿主 SDK 无 chunk 回调，故不是真流式。
- 回归测试新增分两步 / 取消 / 无占位回退用例；无头截图新增骨架态与填字后两张。

## 2026-09-13：小剧场导入器认任意 JSON（随 0.1.1 发布）

- 不认识的 JSON 不再直接报错：`data/scan.js` 按文档顺序递归，短 `name/title/comment/identifier` 当名字、≥24 字含空白或汉字的字符串当正文，跳过头像 / URL / data: 字段，同文去重、上限 400 条；抽屉里勾选，可统一加标签，导成提示词 / 前置要求 / 随机条目。酒馆预设、角色卡 v2（含卡内世界书）、别人的生成器导出都试过。
- 回归测试补一条扫描用例（`check-xiao-ju-chang-regressions.mjs`），无头 Chromium 过一遍抽屉四主题截图与导入 / 取消 / 空 JSON 三条路径。
...
## 2026-09-12：新增小剧场 APP 0.1.0

- 新 APP `custom-apps/xiao-ju-chang/`：单角色小剧场生成器。四类部件各自成库（前置要求、提示词、随机池 + 随机宏、输出方式），一个小剧场是一次组合；每个组合另存记忆三选一、写回开关、我是谁、纯裸通道、随机权重。带 15 条默认提示词、6 条随机、3 个宏、5 个默认组合。
- AI 写的整段 HTML/JS 进沙盒 iframe 直接渲染，iframe 内回传高度，带 `data-action` 的元素被点击后接着那个选择再生成一篇。四套主题换的是整体布局。
- 生成走 `ai.generate` 并用 `promptProfile.include` 白名单只放人设/性格/关系/世界书/选中的记忆标记，聊天记录默认不带；组合可开纯裸通道改走 `ai.chat`，人设和卡内世界书由 APP 自己拼。宏在 APP 内展开。
- 新增 `scripts/build-xiao-ju-chang.mjs` 与 `npm run xjc:build` / `xjc:check`；`public/custom-apps/index.json` 已含本 APP，首次安装包放 `releases/xiao-ju-chang/`。
- 无头自测脚本 `float/tools/shoot-xiao-ju-chang.mjs` 套内存版 SDK 跑通主流程 33 张截图，控制台无报错；同时落盘每次调用的 `promptProfile` 和完整 instruction 供核对。宿主侧代码未改，无云函数变更。
...
### 挂念 0.9.39：变数（2026-09-15）

- 生成TA的一天时同一次调用顺带埋几个「可能的岔子」（日子的起伏：平稳 / 平常 / 多事），挂在具体日程上，到那件事开始后揭晓；发没发生只看「日期|角色|岔子」的固定种子，App 落库、云端读时重算，结果一致，不补报。
- 发生后改写当天后面的安排（插入 / 推迟 / 取消），插入段的忙闲进回复闸门；心情精力记一条会淡掉的情况；聊天提示和云端到点发消息时的状态里写「今天碰上了……」，按说不说分三种口吻。
- 说不说 = 模型倾向按好感挪一档。忍不住：本机约一次心动预约（占额度、守免打扰和睡眠，约不上改成聊到才说）；挂念关着时 push-recheck 的自发起念多一个「碰上事」由头。
...
- 回复闸门的 delay 决定带上 `what`（忙碌窗口或分神的标题），聊天顶部提示改成「TA在上课，15:30 左右再回」「TA在做饭，一会儿就回」；没标题照旧。
- 全局 CSS、聊天 CSS 编辑框停手 250ms 预览，「保存并应用」才落盘，离开未保存则复原；聊天 CSS「清除」只清框。
- 验证：`check-reply-gate` 通过；五个改动文件 eslint 在改动行无新问题，`tsc --noEmit` 无报错。宿主页面没法本机构建，未截图实测。

### 图片预览 · 编辑 · 出镜长相 · 同步差异（2026-09-15）

- 预览层加双指缩放 / 双击 / 拖动，按钮改成编辑 · 重新生图 · 保存；重新生图不弹框，沿用这张的描述和单独正负向。
- 新编辑面板：画面、角色出镜（长相 + 此刻的样子）、画风预设、本次正向 / 负向；只改这一张，存在消息上。
- 长相从人设提取、按人设指纹缓存，人设改了自动重提；此刻的样子由内置预设「发照片」和朋友圈规则原条目要求写成「此刻：…；画面：…」。只加在出镜图上，设置可关。
- 同步内置条目：逐行 + 逐字对比，折叠相同行，条目下写改了几行。
...
## 2026-09-16：挂念后端审阅修复（本地）

- 修复异常路径导致进程退出、outbox 写入失败卡 running、关闭 VPS 模式未停用后端、同轮积压任务绕过发送间隔。
- 增加 HTTP 异常路径、503 重试、响应丢失精确查重、凭据读取失败、同轮间隔及停用切换专项验证。挂念版本 0.10.0；尚未提交、推送或部署。

## 2026-09-16：深度审查 F1 / F6 修复（本地）

- F1：增加旧云端停用交接及分页预约核验，失败不启用后端；已有后端状态保留，新角色只迁入自己的旧计划，本地旧预约在后端模式让位。使用现有 schema 12，无云函数或 schema 变更。
- F6：聊天复核接回情绪与日程输出，状态落库后宿主立即可读；只动未来安排，尊重已有聊天改日程开关，自发起念不凭空改状态。
- 版本在下一条升到 0.10.1。

## 2026-09-17：深度审查 F2～F5、F7～F10 修复（挂念 0.10.1）

- F2：后端启动时把上个进程留下的 running 任务放回 pending；重发前照旧按 `timedwake:<id>` 查投递凭据，已投递的只补记录不重发。开始生成改成原子抢占（`claimTimer`，只抢 pending）。
- F3：`days` 新增 `gen_at`，生成失败的十分钟退避按上次尝试时刻算，不再被每分钟保存的 `updated_at` 推后。
- F4：零点到自动生成之间没有今天的生活面时，发送前按昨天的起床 / 上床（没有就用存下的固定作息、再没有用免打扰时段）判断睡着 / 忙，睡着按原规则押到起床。
- F5 / F10：账本开关独立成 `threadsOn` 设置（进白名单），旧角色没寄过时仍按有无账本数组；新建角色从空账本开始。挂念 `serverSettings` 补寄 `threadsOn` 和 `genEnabled`（= 每天到点自动生成）。
- F7：撤念头、了结 / 删账本、停用角色在排队等锁时立刻挂出标记（`Runner.exclusive(fn, halts)`），引擎在开始生成前和写 outbox 前都查；成文期间收到撤销就放回 pending，让排队的撤销落地。
- F8：生成一天时读最新聊天（60 条）补进指令，自动和手动重新生成同一入口。
- F9：宿主寄来的固定作息原件和例外另存 `routines` 表；今明两天仍用宿主折好的，更远的日子后端自己展开。
- 验证：后端 55 项测试 + tsc；挂念 `gua-nian:check`、`apps:build-dist`。

## 2026-09-16：修复复查发现的三处遗漏（本地）

- 旧挂念预约的创建/快照刷新入口按 VPS 归属退役本地登记，组装后再检查一次；本地发送使用同一归属函数，不碰云端发送凭据。
- 交接对已建档角色补迁缺失任务及关联约定，保留后端状态、终态和改期；失败可重试，旧版已确认但未迁入的角色通过新标记补做交接。
- 日程生成把回看设置传入历史格式化，线下摘要不再被过滤，线上轮数也按设置取。
- 新增真实刷新入口、增量迁入、实际交接路由与线下摘要专项验证。版本未改；未提交、推送、部署。

## 2026-09-17：挂念发送分流迁到后端（来电 / 微信 / 快捷动作 / 安卓壳），复查两处修补

- 后端 `src/delivery.ts`：到点成文后解析来电、快捷动作、发到微信标记（规则搬自 push-generate）。来电推单条 `incoming_call`，20 小时频控改由后端到点决定。微信经微信云助手发送，送不成退回聊天。快捷动作经个人云现实桥网关建命令、先推消息再投递、失败落诊断行。草稿新增分流字段，重启不重复执行动作、不重复发微信。
- 后端 `src/shortcut-resume.ts` + `shortcut_resumes` 表：会回传结果的快捷动作由后端等结果、生成第二轮，不再挂云端 `shortcut_resume` 任务。
- `push.ts`：安卓壳 `shell:` 订阅改发 Realtime 广播，来电带 `kind: "call"`；唤醒后端推送也走同一路。
- 宿主 `armCompanionChatTemplate`：聊天模板加来电占位、快捷动作说明、微信通道和续跑底稿，与定时唤醒一致；`templates.ts` 同步时保留 `weixin` 与 `shortcutContinuation`。
- 修补 GPT 复查改动：旧交接角色补迁回音账失败不再拖停整轮；交接补迁遇到找不到账本的约定改为跳过并记一笔，不再让交接失败（失败会让旧云端和 VPS 都停着）；已撤销念头的成文草稿随撤销删除。
- 验证：后端 80 项测试 + tsc；`check-gua-nian-server-handoff`、`check-gua-nian-bailout-ownership`、`check-wake-server`；根 tsc；eslint。线上只读核对了交接补迁和回音补迁对现有两个角色无报错、Realtime 广播与现实桥网关可达。未做手机实测。
