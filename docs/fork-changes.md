...
- **重试先生成后删**（`components/chat/chat-room.tsx`）：线上 `handleRetry` 与线下 `handleOfflineRetryFrom` 都不在点下去的当下删旧回复。线下等新一轮拿到手才把「基底 + 新一轮」整份一次写入；线上先记下要替换的 id，由 `runManagedGeneration` 的 `onSaved`（比对写入前后的消息条数，真的变多才触发）在落盘后 `deleteChatMessagesByIds` 删掉它们。**版本记录照旧在重试前就写好，换版本能力不变**，这里改的是时间线：中断、断网、报错、沉默、空回复，乃至 iOS 把页面整个回收（catch 根本不执行）时，旧回复都留在原位，不用去版本选择器捞。没产出时两条路径都 `rollback()` 版本记录并从存储把界面读回来。**以后写这类「替换掉某一轮」的流程，一律先生成后落盘**，不要先删掉占位再指望回滚。
...
### 唤醒后端：事件唤醒交给 VPS（2026-09-17）

工具箱 MCP 的事件唤醒新增「交给 VPS 后端」：事件网关把这类来源的事件只交给 `companion-server` 的唤醒后端，手机关着也由后端跑回复和绑定 MCP 的工具，经 `push_outbox` + Web Push 送回，补收时落事件原文和动作记录。小手机给每个这样的来源冻底稿（聊天提示词 + 完整记录 + 事件占位 + 只含该 MCP 的工具与凭据）寄给后端。花园仍需手动启动、断线即停，交给后端时断线会推送提醒。详见 `companion-server/README.md`「唤醒后端」。
...
挂念交接补迁：`importMissingCloudTasks` 按 wakeId 补入已建档角色缺失的交接任务和关联约定，以云端交接取消标记及 outbox 为凭据；保留后端修改与终态。`handoff-tasks` 标记要求旧版交接补做迁入后才算完成。宿主发送/快照刷新共用 `guanian-wake-ownership.ts`，VPS 模式移除旧本地登记、不重挂云端；日程生成按设置保留独立线上轮次和线下摘要。


挂念迁移收尾：VPS 生成后先把正文、原生成时间和固定 outbox ID 存入 SQLite `generated_drafts`，投递失败/重启复用正文并核对发送凭据；生成时间与投递时间分开，回音和主动间隔按投递计时；影子模式不投递草稿，停用、取消与约定版本继续拦截。交接补迁旧消息未结算回音账，已有交接角色自动补查；`fbSeen` 和 `sends.wake_id` 防重，累计计数与结算标记原子落盘。挂念与事件唤醒复用 schema 12 会话生成租约，与旧云端其他任务互斥；取得锁后刷新历史，长调用自动续租，事件等待锁时不提前 ack。不改安装包版本或云函数。详见 `companion-server/README.md`。


挂念分流恢复：快捷命令一律延后投递，正文与动作投递分阶段持久化，outbox 响应丢失后补投同一命令；通知确认后才激活结果续跑。所有外部动作及续跑共用角色/会话/取消/租约检查。微信明确失败固定转投聊天；请求超时、5xx、坏回执或进程中断的结果不明状态保留正文待核对，不自动重发或改投。结果续跑拿锁后刷新时间和聊天事实，生成正文先存 SQLite，再独立重试交付；停用暂停不消耗失败次数，重试耗尽保留恢复记录。仅修改 companion-server，不改云函数、schema 或挂念安装包版本。
