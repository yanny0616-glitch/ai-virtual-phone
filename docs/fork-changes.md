...
- **通知横幅编辑器**（外观 →「通知横幅」，`components/theme/notif-banner-page.tsx`）：五种场景（单聊 / 群聊 / 系统 / 长文本 / 来电）用横幅真身的 class 实时预览，浅深色切换、重播进场；逐项调 `--notif-*` 存进 `cssOverrides`，带拍立得 / 窄条 / 居中卡片三套预设。新增 `--notif-duration` 控制停留 1.2–30 秒。全局一份，不按角色分配。
