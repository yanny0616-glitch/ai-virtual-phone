...
### 拾光独立 APP 与可见回忆摘要（2026-09-07）

- `custom-apps/shiguang` 提供可安装 ZIP：按角色搜索、分类/发送方式筛选、原消息回看、事实/摘要/关键词编辑、删除确认、手动整理和全局拾光设置。记忆库原拾光页面及设置区改为打开 APP 的入口；已有记录、自动整理进度和配置原地接续，打开 APP 不复制或改写记录。
- 宿主新增受独立权限保护的拾光 SDK，`lib/shiguang-app-api.ts` 只允许管理指定角色的拾光记录和四个拾光设置项。编辑和删除使用事务内版本检查，防止旧编辑覆盖后台整理或另一页面的新内容；删除标记继续阻止自动重建。自动整理和聊天注入继续由宿主执行，APP 关闭仍工作。
- 摘要统一成可查看、编辑的 `promptSummary`。老记录从卡片事实生成默认值，保留人名、具体作品/物品、经过和细节，不发送独立隐藏简述。`promptText` 与聊天共用同一生成函数，包含状态和后续；合并候选也使用可见摘要。编辑后的摘要由用户维护，详情修改不会暗中覆盖摘要，可显式从卡片重新生成。
- `recallMode` 提供预算内优先、按话题/日期、不发送三种方式；默认继承旧记录是否有稳定信息。预算按完整发送文字计，超限整条跳过并在卡片提示，不把卡片标成含糊的「本轮」。具体请求的实际内容继续查看该次提示词查看器/日志。
- 构建/打包见 `shiguang:build` / `shiguang:package`；架构见 `custom-apps/shiguang/ARCHITECTURE.md`。专项测试覆盖真实领域/接口、SDK 权限、IndexedDB 并发覆盖保护和独立 HTML 手机布局。需配套发布宿主再安装 APP；不需更新个人云函数或 SQL。
