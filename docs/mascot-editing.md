...
1. 读取返回 `read={scope,id?,revision}`，并把真实读取记录保存在本轮共享的 `MascotToolContext.editReads`。组件和布局共用 desktop 完整版本，另记录排除组件 config 的结构版本（仍含所有模板源码、位置与结构）；角色更新按单个角色版本，新建使用 characters 列表版本，主题使用 appearance 版本。只有 character 范围保留 id，其余范围忽略模型误填的 id。
2. `准备修改` 接收 title 和最多 50 个 operations；宿主自动选取本轮记录中操作涉及的读取范围，不依赖模型复制版本。reads 字段仅为兼容旧调用保留，非必填且不作为读取依据；没有实际读取、读错角色或读取后相关数据变化仍拒绝。对不写 config、不删除组件的计划，只有 config 变化时可继续，并保留最新配置。原生 schema 的 patch/templatePatch 使用 JSON 对象字符串，执行器也兼容文本协议的对象形式。纯函数生成 deltas，只保存草稿，不改变目标内容。多动作先全部校验，图标交换只检查最终状态。模板改尺寸放不下会拒绝整个方案，不自动移除桌面实例。
...
天气缓存、用户图片等可经 DIY 桥写入实例 config。只修改模板、复制单实例样式、创建/摆放或移动组件等不写 config、不删除组件的操作，读取记录允许按结构版本校验。结构版本排除全部实例 config，但仍包含完整布局、模板源码、组件 ID/类型/尺寸/位置及其他结构。提交/撤销时按实例 ID 带入最新 config，不回写预览快照中的旧配置。

显式 config 写入、删除实例或模板不启用此规则，配置变化时仍要求重读。撤销新增组件会检查即将删除的实例配置；若用户已补充图片或脚本已保存新数据，拒绝删除。历史草稿缺少 preserveWidgetConfig 标记或读取缺少 structureRevision 时沿用完整版本校验。IndexedDB 原子比较与写入保护继续生效，不改变后台刷新频率。

错误现在区分本轮缺少读取、配置在读取后变化、其他相关内容变化；仅凭旧截图无法知道是哪种，更不能据此认定用户实际改过桌面。
