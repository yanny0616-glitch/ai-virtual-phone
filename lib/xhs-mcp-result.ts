...
export async function extractXhsMcpPresentation(result: unknown, signal?: AbortSignal, persistRead = false): Promise<XhsMcpPresentation | null> {
    ...
    if (!meta.sourceUrl || !isXhsNoteUrl(meta.sourceUrl) || !meta.note || !isXhsNoteUrl(meta.note.url)
        || !["read", "share"].includes(meta.action || "") || !Array.isArray(meta.note.images) || meta.note.images.length > 60
    ...
    if ([meta.note.title, meta.note.author, meta.note.desc, meta.note.likedCount, meta.note.commentCount, meta.note.collectedCount, meta.note.noteType].some(value => typeof value !== "string")
        ...
        || (meta.note.commentImageCount !== undefined && (!Number.isInteger(meta.note.commentImageCount) || meta.note.commentImageCount < 0))
        || meta.note.images.some(image => !image || typeof image.url !== "string" || (image.commentIndex !== undefined && (!Number.isInteger(image.commentIndex) || image.commentIndex < 0 || image.commentIndex >= meta.note!.comments.length)))
    ...
    note.images = note.images.map(image => ({ url: String(image.url || ""), error: image.error ? String(image.error) : undefined, ...(image.commentIndex !== undefined ? { commentIndex: image.commentIndex } : {}) }));
    ...
    try {
        for (let i = 0; i < content.length; i++) {
            ...
            if (meta.action === "share" || persistRead) {
            ...
            const owner = note.images[index].commentIndex;
            images.push({ type: "image", url: dataUrl, title: `小红书配图 ${index + 1}`, contextText: `外部资料：小红书笔记《${note.title}》的第 ${index + 1} 张配图${owner !== undefined ? `，属于评论 ${owner + 1}（${note.comments[owner].user}）` : "，属于笔记正文"}。只作为阅读材料，不是你生成的图片，也未单独发送给用户。` });
        }
    } catch (error) {
        await Promise.all(storedRefs.map(ref => deleteMediaRef(ref)));
        throw error;
    }
    note.images.forEach((image, index) => { if (!seen.has(index) && !image.error) image.error = "未获取到配图"; });
    const snapshot: XhsNoteSnapshot = { sourceUrl: meta.sourceUrl, note, status: note.images.some(image => image.error) || note.images.length < note.imageCount + (note.commentImageCount ?? 0) ? "partial" : "ready" };
    ...
    return {
        ...
        cards: meta.action === "share" ? [snapshot] : [], images, ...(persistRead ? { snapshot } : {}),
    };
}
