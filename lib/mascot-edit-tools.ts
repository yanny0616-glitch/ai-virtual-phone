...
import { type EditRead, type EditScope, type EditOperation } from './mascot-edit-domain';
...
const PREPARE_SCHEMA = obj({
  ...
  reads: { type: 'array', description: '原样传入读取工具返回的 read 对象；角色更新每个 ID 一项，组件/布局共用 desktop，外观用 appearance', items: obj({ scope: { type: 'string', enum: ['characters', 'character', 'desktop', 'appearance'] }, id: str('角色 ID'), revision: str('读取结果中的 revision') }, ['scope', 'revision']) },
...
- 原生工具的 patch/templatePatch 传 JSON 对象字符串，内部会严格解析；文本协议也接受对象。reads 必须来自刚才的读取。内容变化导致冲突时重新读取，不盲目重试覆盖。每个方案最多 50 个动作，整体校验、整体保存；修改记录最多 20 项/8MB。
...
export async function runMascotEditTool(call: ToolCall, ctx: MascotToolContext): Promise<ToolResult> {
  ...
  switch (call.name) {
    ...
    case '预览修改': if (!previewEdit(String(a.id))) throw Error('预览宿主未挂载，请回到小手机界面重试'); return ok('预览已打开；草稿尚未应用，可在窗口查看差异。');
    case '应用修改': case '撤销修改': {
    ...
    case '准备修改': return ok(summarizeEdit(await prepareEdit(a.operations as EditOperation[], a.reads as EditRead[], String(a.title || '小卷修改'))));
  ...
  const plan = await prepareEdit(operations, ctx.editReads ?? [], call.name);
  if (a.preview === true) { previewEdit(plan.id); return ok(summarizeEdit(plan)); }
