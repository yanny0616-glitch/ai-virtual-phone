import { readDeferredReply, writeDeferredReply, readReplyGate, listDeferredReplySessions, type DeferredReply } from "./chat-reply-gate";
...
export function freezeDeferredReplyTiming(rec: DeferredReply): CloudReplyTiming {
    const gate = rec.characterId ? readReplyGate(rec.characterId) : null;
    ...
    return {
        nextAt: rec.until, reason: rec.reason, windowKey: rec.busyWindowKey,
        ...
        windows: (busy?.windows ?? []).map(win => ({
            from: dateAt(busy!.date, win.from), to: dateAt(busy!.date, win.to), title: win.title,
            key: JSON.stringify([busy!.date, busy!.adaptive === true, busy!.peekMin, busy!.focusedPeekProb ?? 0, win]),
            focused: /开会|会议|例会|晨会|周会|月会|上课|课堂|考试|测验|开车|驾驶|手术|面试|汇报|训练|排练|实验|演出|上台/.test(win.title),
            breaks: (win.breaks ?? []).map(p => ({ from: dateAt(busy!.date, p.from), to: dateAt(busy!.date, p.to) })),
        })),
    };
}

type Receipt = { ok?: boolean; supported?: boolean; status?: string; executeAt?: string; resultNote?: string; error?: string };
...
async function sync(sessionId: string): Promise<void> {
    ...
    try {
        ...
        if (!rec.cloud.attempted) {
            ...
            if (!subscription || !capability?.supported) {
                const latest = current();
        ...
        if (latest?.cloud) {
            ...
            if (firstError) notify(sessionId, "离线等待同步未确认，请保持小手机开启以重试；暂不另发，避免重复回复");
        }
...
export function installDeferredReplyCloudSync(): void {
    ...
    const refresh = () => { for (const id of listDeferredReplySessions()) queueDeferredReplyCloud(id); };
    for (const event of ["settings-api-configs-updated", "settings-bindings-updated", "settings-presets-updated", "reply-gate-updated"]) window.addEventListener(event, refresh);
