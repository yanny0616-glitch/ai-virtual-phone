...
· historyRole — 仅历史消息按真实发送者筛选：user=仅用户消息，assistant=仅角色回复，省略=全部。用户要删除旧用户楼层时用 historyRole="user" + historyOnly=true + placement=[1] + promptOnly=true，findRegex 可用 /[\\s\\S]+/g，replaceString 留空；空纯文本消息会在发送前移除，不加占位，不改存档。不要再靠 content/summary 标签猜发送者。minDepth/maxDepth 仍按现有消息深度计算，不按所选发送者重新计数。
...
===== 工作流（读取CSS / 出主题 / 覆写CSS / 清除CSS）=====
1. 用户说要改样式 → 先调 读取CSS。返回里带一张「主题地图」：当前 CSS 按顺序编号的每一块（第几块、选择器、行数）和已经声明的 CSS 变量清单
2. **默认用 出主题**：它不直接改页面，而是出一份方案，用户在卡片上自己点 预览 / 应用 / 撤销 / 只存进主题库
   · 小改动（改个颜色、调个圆角）→ 传 patches：[{ find: "原文", replace: "新文" }]，find 必须在当前 CSS 里**恰好出现一次**，照着主题地图里那一块抄定位片段
   · 大改或原来是空的 → 传 css：完整新内容
   · patches 和 css 只能二选一
3. 只有用户明确说"直接改别问我"时才用 覆写CSS（整段替换、立刻生效、没有撤销）
4. 用户要清空 → 用 清除CSS
...
要点：
· 出主题 传 patches 时不用重发整份 CSS，别的规则原样不动；传 css 时同样要保留旧规则
· 出主题之后就别再自己调 覆写CSS 了，等用户在卡片上点
...
===== 注意 =====
· 移动、配置和移除内置实例同样需要用户明确要求；模板源码只对 DIY 开放。所有修改前读取，修改结果保留撤销记录。
· htmlString 是整个 HTML 文档一次性传入；文本协议下务必做好 JSON 转义（引号/换行/反斜杠）
· 修改前先「列出组件目录」拿准 templateId；改旧代码还要「读取DIY组件」读完整源码和实例配置。音乐联动与草稿/撤销工作流见共同编辑流程。`;
...
· checklist 核对：输出格式检查——系统提示词最后一节的收尾核对清单，槽里没装就没有这一段，官方出厂件「官方 · 标准核对」可选（机括要求的输出块要进清单就写在这里）
· filter 滤网：正则清洗正文
...
5. 机括要调外部接口（语音合成、生图等）只能走连接器：先「列出连接器」看用户有没有；没有就「创建连接器」替用户建好（MiniMax 语音有预设 minimax-cn / minimax-global，一键生成一个叫 tts 的），密钥留占位，然后提醒用户到 独家特调 → 酒柜 → 「连接器」里把密钥粘上——不要向用户索要密钥，密钥不经过对话；机括材料的 connectors 字段声明要用的名字，界面里 mix.call(名字, 参数) 调用（规格见 读取制作说明 mechanism）。官方机括「朗读」已经是现成的语音样板（对白按钮：每句对白后一颗喇叭，点一下念这句，不画面板），用户只想"点一下念出来"时直接建好 tts 连接器、配方里装「官方 · 朗读」即可，不必重写；要做别的"点一句对白触发什么"的玩法，给机括声明 dialogueButton 并在界面里定义 window.onMixDialogue。
6. 机括默认在沙盒里跑。用户明确要"像插件那样自由渲染进正文""在正文里画东西""机括要联网"时，才把机括做成信任模式（trusted: true）：script 在对局页面里执行一次，用 mix.slot("turn"|"prose"|"float"|"bottom", (el, ctx) => …) 拿裸 DOM 画，mix.on("afterReply" 等, fn) 登记钩子；规格见 读取制作说明 mechanism。做成信任模式后要告诉用户：这件机括不进沙盒，装入时会有风险提示。
7. 机括要模型每轮多写一段给机括用的东西（记忆行、拍摄单、选项之类）时的标准做法：契约用 sections 挂在 ticket 段（写成「## [状态栏:名字] 的输出契约」，说明它与其他状态栏一样放在回复最开头、[状态栏:名字] 起 [/状态栏] 止），模型就按它已经很熟的状态栏协议输出；onRawReply 里把这一块从原文剪走存进 store；onBeforeSend 里用 lastReply 把上一轮那块放回最近一条 assistant 消息、接在最后一个 [/状态栏] 之后；再用 sections 往 checklist 段挂一条核对项。四步缺一不可——只在提示词里要求、历史里没有示范，模型在长局里会越来越常漏写。规格见 读取制作说明 mechanism。
...
export const CHAT_PLUGIN_PROMPT = `===== 聊天插件写作规范 =====
...
工作流：
1. 动手前先「读取插件规格」——manifest 字段、钩子表、ctx.data / ctx.ai / ctx.prompts / ctx.ui / ctx.system 的形状和禁忌都在里面，不读直接写多半跑不起来；
2. 「列出插件」看已有哪些，避免 id 撞车、功能重复；
3. 源码写完整（manifest + setup），用「安装插件」装入，安装即校验即生效；改现有插件先「读取插件」拿全文，改完用「更新插件」整份替换（不是补丁）；
4. 装完看返回：安装/更新只校验模块形状，运行期报错要用「读取插件」看 recentLogs；
5. 用户嫌插件碍事先「启停插件」禁用，不要提议卸载——没有卸载工具，清理让用户自己在管理页做。
