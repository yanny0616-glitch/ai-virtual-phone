...
- Generate a random private backend credential at `/etc/float-garden-wake/backend-token`, directory 0700, file 0600. Never log it.
...
- Next.js `/api/garden-wake` requires self-hosted mode and same-origin browser POST, and forwards to the credential-protected loopback service. This is a single-owner VPS integration, not a multi-tenant service.
